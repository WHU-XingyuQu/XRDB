...
pub enum Token {
    Keyword(Keyword),
    // String Token，like name of chart and column
    Ident(String),
    // data in the type of string
    String(String),
    // number
    Number(String),
    // (
...
// Iterator to get the next token
...
impl<'a> Lexer<'a> {
    ...
    fn scan_number(&mut self) -> Option<Token> {
        ...
        // if a '.' exists, it turns out it's a float
    ...
    fn scan_ident(&mut self) -> Option<Token> {
        ...
        Some(Keyword::from_str(&ident).map_or(Token::Ident(ident.to_lowercase()), Token::Keyword))
    }
